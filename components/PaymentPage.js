...
const PaymentPage = ({ username }) => {
  ...
  const getData = async () => {
    ...
    setcurrentUser(u); // set the current user state with the fetched user
    let dbpayments = await fetchpayments(username);
    setPayments(dbpayments); // set the payments state with the fetched payments
    console.log(dbpayments);
    console.log(u);
  };

  const pay = async (amount) => {
    //get the order id from the server by calling an api
    ...
    var options = {
      ...
      currency: "INR",
      name: "LaunchPad", //your business name
  ...
  return (
    <>
      ...
      <div className="info flex justify-center items-center my-24 mb-32 flex-col gap-1">
        <div className="font-bold text-lg">@{username}</div>
        <div className="text-slate-400">Every small support takes {username} closer to launch 🌍</div>
